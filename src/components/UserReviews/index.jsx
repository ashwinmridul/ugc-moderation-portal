...
export default ({uidx}) => {
    ...
    const fetchReviews = () => {
        setLoading(true);
        fetch(`/getReviewsByUser/${uidx}?page=${page + 1}&size=${PAGE_SIZE}`).then(response => response.json()).then(response => {
            setLoading(false);
            setData(response.reviews);
        });

        // pagination call with a max cap of 1000 reviews, i.e., 100 pages
        fetch(`/getReviewsByUser/${uidx}?page=${page + 1}&size=${1000}`).then(response => response.json()).then(response => {
            if (!totalReviews) {
